...
async def insert_log(log_data: dict) -> str:
    ...
    if await _is_mongo_up():
        result = await _real_db.logs.insert_one(doc)
...
async def get_logs(limit: int = 50, skip: int = 0,
                   severity: str = None, threat_label: str = None,
                   user_id: str = None) -> tuple[list, int]:
    ...
    if await _is_mongo_up():
        ...
        total  = await _real_db.logs.count_documents(query)
        cursor = _real_db.logs.find(query).sort("created_at", -1).skip(skip).limit(limit)
        logs   = []
...
async def get_log_by_id(log_id: str) -> Optional[dict]:
    ...
    if await _is_mongo_up():
        try:
            doc = await _real_db.logs.find_one({"_id": ObjectId(log_id)})
            if doc:
                doc["_id"] = str(doc["_id"])
            return doc
        except Exception:
...
async def get_analytics() -> dict:
    ...
    if await _is_mongo_up():
        threat_dist = []
        async for doc in _real_db.logs.aggregate([
        ...
        risk_over_time = []
        async for doc in _real_db.logs.find({}, {"risk_score": 1, "created_at": 1, "_id": 0}).sort("created_at", -1).limit(100):
        ...
        risk_over_time = [{"risk": l.get("risk_score", 0), "time": l.get("created_at")} for l in logs[-100:]]
        ...
        top_ips   = sorted([{"ip": k,   "avg_risk": round(sum(v)/len(v), 1)} for k, v in ip_risk.items()],   key=lambda x: -x["avg_risk"])[:8]
        top_users = sorted([{"user": k, "avg_risk": round(sum(v)/len(v), 1)} for k, v in user_risk.items()], key=lambda x: -x["avg_risk"])[:8]
        return {
            "threat_distribution": threat_dist,
            "severity_distribution": severity_dist,
...
async def save_explanation(log_id: str, explanation: dict) -> None:
    ...
    if await _is_mongo_up():
        try:
            await _real_db.logs.update_one(
                {"_id": ObjectId(log_id)},
...
async def get_explanation(log_id: str) -> Optional[dict]:
    ...
    if await _is_mongo_up():
        try:
            doc = await _real_db.logs.find_one(
                {"_id": ObjectId(log_id)},
                {"explanation": 1}
            )
            return doc.get("explanation") if doc else None
        except Exception:
            return None
    else:
...
async def get_settings() -> dict:
    if await _is_mongo_up():
        doc = await _real_db.settings.find_one({"_id": "global"})
        if doc:
            doc.pop("_id", None)
            return doc
...
async def save_settings(data: dict) -> None:
    if await _is_mongo_up():
        await _real_db.settings.update_one(
            {"_id": "global"},
            {"$set": data},
            upsert=True
        )
...
async def save_alert(alert_doc: dict) -> None:
    ...
    if await _is_mongo_up():
        await _real_db.alerts.insert_one(doc)
    else:
...
async def get_alert_history(limit: int = 20) -> list:
    ...
    if await _is_mongo_up():
        cursor = _real_db.alerts.find({}).sort("fired_at", -1).limit(limit)
        docs = []
        async for doc in cursor:
            doc["_id"] = str(doc["_id"])
            docs.append(doc)
